...
class VstatHttpClient extends VstatClient with RankedDomainJsonProtocol {
  def getTrafficCount(domain: String): Future[VstatData] = {
    ...
    val request = HttpRequest(uri = "https://web.vstat.info/" + domain).withHeaders(firstHeader, secondHeader)
